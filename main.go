...
func getAllMahasiswa(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
		err := rows.Scan(&mahasiswa.Id, &mahasiswa.Npm, &mahasiswa.Nama, &mahasiswa.Kelas, 
			&mahasiswa.Profile)
		if err != nil {
			log.Print(err.Error())
		} else {
...
func getMahasiswaByName(w http.ResponseWriter, r *http.Request) {
	...
	for rows.Next() {
	if err := rows.Scan(&mahasiswa.Id, &mahasiswa.Npm, &mahasiswa.Nama, &mahasiswa.Kelas,
		 &mahasiswa.Profile); err != nil {
	log.Print(err.Error())
	}
	mhs = append(mhs, mahasiswa)
	}
